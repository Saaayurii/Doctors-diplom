...
function EditProfile() {
  ...
  const formFields = [
    ...
    { name: "birthDate", title: "Год рождения", type: "number" },
    { name: "residenceCountry", title: "Страна проживания", type: "text" },
    { name: "specialization", title: "Специализация", type: "text" },
    {
      name: "languages",
      title: "Языки (разделяйте пробелами)",
      type: "text",
    },
    { name: "thirtyMinPrice", title: "Цена за 30 минут", type: "number" },
    { name: "sixtyMinPrice", title: "Цена за 60 минут", type: "number" },
  ];
  ...
  const validateFirstName = () => {
    ...
    if (formData.firstName && !regex.test(formData.firstName)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        firstName: "Имя должно состоять только из букв",
      }));
    } else {
      if (errorMessage.firstName !== "") {
        changedValidation = true;
  ...
  const validateLastName = () => {
    ...
    if (formData.lastName && !regex.test(formData.lastName)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        lastName: "Фамилия должна состоять только из букв",
      }));
    } else {
      if (errorMessage.lastName !== "") {
        changedValidation = true;
  ...
  const validateBirthDate = () => {
    ...
    if (formData.birthDate) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        birthDate: "Требуется дата рождения.",
      }));
    }
  ...
  const validateResidenceCountry = () => {
    ...
    if (formData.residenceCountry && !regex.test(formData.residenceCountry)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        residenceCountry: "Страна должна состоять только из букв",
      }));
    } else {
      if (errorMessage.residenceCountry !== "") {
        changedValidation = true;
  ...
  const validateSpecialization = () => {
    ...
    if (formData.specialization && !regex.test(formData.specialization)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        specialization: "Специализация должна состоять только из букв",
      }));
    } else {
      if (errorMessage.specialization !== "") {
        changedValidation = true;
  ...
  const validateLanguages = () => {
    ...
    if (formData.languages && !regex.test(formData.languages)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        languages: "Языки должны состоять только из букв",
      }));
    } else {
      if (errorMessage.languages !== "") {
        changedValidation = true;
  ...
  const validateThirtyMinPrice = () => {
    ...
    if (formData.thirtyMinPrice && !regex.test(formData.thirtyMinPrice)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        thirtyMinPrice: "Цена должна быть числом",
      }));
    } else {
      if (errorMessage.thirtyMinPrice !== "") {
        changedValidation = true;
  ...
  const validateSixtyMinPrice = () => {
    ...
    if (formData.sixtyMinPrice && !regex.test(formData.sixtyMinPrice)) {
      ...
      setErrorMessage((prevError) => ({
        ...prevError,
        sixtyMinPrice: "Цена должна быть числом",
      }));
    } else {
      if (errorMessage.sixtyMinPrice !== "") {
        changedValidation = true;
