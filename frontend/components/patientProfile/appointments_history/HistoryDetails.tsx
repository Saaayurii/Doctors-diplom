    ...
    const fetchAppointmentsDetails = async () => {
      if (appointment?.appointment_id) {
        ...
        try {
          ...
          setError("Ошибка при загрузке деталей записи");
        } finally {
          setLoading(false);
        }
      }
          ...
          >
            <div className="my-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
              ...
              <div>
                ...
                {appointmentDetails.appointment_duration} мин
              </div>
              ...
              {appointmentDetails.appointmentResults &&
                appointmentDetails.appointmentResults.length > 0 && (
                  <div>
                    <strong>Результаты приёма:</strong>
                    {appointmentDetails.appointmentResults.map(
                      (result: any, index: number) => (
                        <p key={index}>
                          Диагноз: {result.appointment_diagnosis} <br />
                          Отчёт: {result.appointment_report} <br />
                        </p>
                      ),
                    )}
                  </div>
                    ...
                    {appointmentDetails.medications.map(
                      (med: any, index: number) => (
                        <p key={index}>
                          ...
                          С {formatDate(med.medication_start_date)} по{" "}
                          {formatDate(med.medication_end_date)} <br />
                          Примечание: {med.medication_note}
                        </p>
                      ),
                    )}
                  </div>
